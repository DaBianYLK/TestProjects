...
void GetSysInfo::GetCpuInfo(string &chProcessorName, string &chProcessorType, DWORD &dwNum, DWORD &dwMaxClockSpeed)
{
	...
	string strPath = _T("HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0");//注册表子键路径
	CRegKey regkey;//定义注册表类对象
	LONG lResult;//LONG型变量－反应结果
	lResult = regkey.Open(HKEY_LOCAL_MACHINE, strPath.c_str(), KEY_ALL_ACCESS); //打开注册表键
...
void  GetSysInfo::GetMemoryInfo(string &dwTotalPhys, string &dwTotalVirtual)
{
	...
	sprintf_s(g_szBuffer, "物理内存:%ld MB", dwSize);
	dwTotalPhys = g_szBuffer;

	sprintf_s(g_szBuffer, "虚拟内存:%ld MB", dwVirtSize);
		...
		for (int a = 0; a<(int)dataBlockPtr->NumObjectTypes; a++)
		{
			...
			if (objectPtr->ObjectNameTitleIndex == 510)
			{
				...
				for (int b = 0; b<objectPtr->NumInstances; b++)
				{
					...
					Interfaces.push_back(string(pName)); //各网卡的名称
...
void GetSysInfo::GetDiskInfo(DWORD &dwNum, string chDriveInfo[])
{
	...
	for (int i = 0; i<DSLength / 4; ++i)
	{
		...
		if (fResult)
		{
			sprintf_s(g_szBuffer, "磁盘总容量%fMB", (float)i64TotalBytes / 1024 / 1024);
			strTotalBytes = g_szBuffer;

			sprintf_s(g_szBuffer, "磁盘剩余空间%fMB", (float)i64FreeBytesToCaller / 1024 / 1024);
